...
db_port = 3306
db_name = 'judicial_doc_measurement'
...
# put all front-end domains & outer service domains here
api_allow_origins = ['http://mooctest.whiskychoy.com',
                     'http://localhost:8066',
                     'http://localhost:5500',
                     'http://assess.ngrok.whiskychoy.com:8081']

# home_dir = os.path.expanduser("~")
# usr_cwd = os.path.join(home_dir, 'judicial_doc_measurement')
usr_cwd = '.'

web_data_path = os.path.join(usr_cwd, 'web_data')
# web_data_path = 'static/web_data'
# 上传文件默认路径
upload_base_dir = os.path.join(web_data_path, 'upload_files')

# 报告保存默认路径
writ_report_base_dir = os.path.join(web_data_path, 'writ_report')

# 分析报告默认路径
task_report_base_dir = os.path.join(web_data_path, 'task_report')

# 标准指标字典json父路径
transfer_config_dir = os.path.join(usr_cwd, 'config')

# 标准指标字典json路径
transfer_config_path = os.path.join(transfer_config_dir, 'transfer_config.json')

data_path = os.path.join(usr_cwd, 'data')

ay_path = os.path.join(data_path, 'AY.txt')
